...
def resultado_votacao():
    ...
    for CARGO in range(len(pathCargoFile)):
        with open(cargoVtos[CARGO],"r",encoding="utf8") as cargoFile:
            ...
            totalVotosValidos_list = totalDeVotos_list[:-2]
            totalDeVotos = int(totalDeVotos_list[0])
            totalVotosValidos = int(totalDeVotos_list[0])
            for voto in totalDeVotos_list[1:]:
                totalDeVotos = totalDeVotos + int(voto)
            for voto in totalVotosValidos_list[1:]:
                totalVotosValidos = totalVotosValidos + int(voto)

        cargo_atual = CARGO
        candidatosLISTA = lista[0].split(";")
        candidatos = candidatosLISTA[:-3]
        
        colocacao_list = []
        for i in range(len(candidatos)):
            nome = candidatos_legiveis[candidatos[i]]
            votosRecebidos = int(totalVotosValidos_list[i])
            if totalDeVotos != 0:
                porcentagem = votosRecebidos/totalVotosValidos*100
                porcentagem = '{:.2f}%'.format(porcentagem)
            else:
                porcentagem = '0%'
            colocacao_list.append((nome,votosRecebidos,porcentagem))
            
        colocacao_list.sort(key=lambda x: x[2], reverse=True)
        if CARGO == 0:
            resultadoSenador = open(caminho + "/data/candidatos/votos/senador_resultado.csv",'w',encoding='utf-8')
            resultadoSenador.write("Nome;Total de Votos Recebidos;Porcentagem de Votos\n")
            ...
            resultadoSenador.write('Branco;'+str(totalDeVotos_list[-2])+'\nNulo;'+str(totalDeVotos_list[-1])+'\n')
            resultadoSenador.write("Total de votos válidos;"+str(totalVotosValidos)+"\nTotal de votos;"+str(totalDeVotos))
            resultadoSenador.close()
        elif CARGO == 1:
            resultadoPresidente = open(caminho +"/data/candidatos/votos/presidente_resultado.csv",'w',encoding='utf-8')
            resultadoPresidente.write("Nome;Total de Votos Recebidos;Porcentagem de Votos\n")
            for candidatoResultado in range(len(colocacao_list)):
            ...
            resultadoPresidente.write('Branco;'+str(totalDeVotos_list[-2])+'\nNulo;'+str(totalDeVotos_list[-1])+'\n')
            resultadoPresidente.write("Total de votos válidos;"+str(totalVotosValidos)+"\nTotal de votos;"+str(totalDeVotos))
